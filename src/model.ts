...
import {createConditionsBuilder} from "./conditions";
...
export abstract class Model {
  static find<T>(whereBuilder: WhereBuilder<T>, limit?: number) {
    ...
    whereBuilder((field) => createConditionsBuilder(conditions, field));
    return {
      async fields(rawFields: SelectFields<T>) {
        const fields = Query.fields(rawFields);
        return await Database.query(
          `SELECT ${fields} FROM ${table} ${Query.where(
            conditions,
          )} ${Query.limit(limit)}`,
        );
      },
  ...
  static async first<T>(
    ...
    const fields = Query.fields(rawFields);
    return await Database.query(
      `SELECT ${fields} FROM ${table} ${Query.orderBy<T>(
        table,
        orderBy,
      )} ASC ${Query.limit(count)}`,
    );
  }
